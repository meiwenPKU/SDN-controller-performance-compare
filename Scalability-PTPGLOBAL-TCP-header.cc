 ...
 * P2P SDN. each controller has the global version of the whole network.
 *
...
//--------------define the network it controls
 ...
 double Diameter = 3; // the average number of switches a flow going through in one domain
 int numControllers = 3; //  the number of controllers in the network
 double Diameter_controller = 1.333; // the average number of controllers a flow going through
...
public:
	...
	void SendStatsRes(uint32_t PacketID, uint16_t sendID)
	{
	...
	void ReceivePacket (Ptr<Socket> socket)
	{
	  ...
	  if (destinationHeader.GetPacketType() == FlowMod)
	  {
		  ...
		  /*
		  if (destinationHeader.GetSenderID() == m_id)
		  {
			  while (true)
			  {
				  if (destinationHeader.GetPacketID() == m_QSentFlowInitial.front())
				  {
					  m_vDelay.push_back(Simulator::Now().GetSeconds()- m_QLastSend.front());
					  m_QSentFlowInitial.pop();
					  m_QLastSend.pop();
					  break;
				  }
				  else
...
public:
	...
	uint16_t m_id; // the id of the controller
	static std::vector<Controller*> m_vController;
...
public:
	Controller(uint32_t V, uint32_t diameter, Time ReqInterval): m_V(V), m_diameter(diameter), m_lastSend(0),
	m_ReqInterval(ReqInterval),m_num_flow_setup (0), m_num_flow_remove (0),
	m_num_status_report(0), m_num_flow_mod(0), m_num_status_request(0), m_id(0), m_id_status_req(1){
		//m_vController.push_back(this);
	}
	/*
	...
	void SendReq()
	{
				...
				it != m_vSendSocket.end(); ++it)
		{
		    ...
		    MyHeader sourceHeader;
		    sourceHeader.SetData(StatsReq,m_id,m_id_status_req);
	...
	void HandleRead(Ptr<Socket> socket)
	{
		...
		if (destinationHeader.GetPacketType() == FlowInitial)
		{
			...
			NS_LOG_INFO( Simulator::Now().GetSeconds() << " Controller " << m_id << " receives a initial flow setup packet");
			m_num_flow_setup++;
			for (int i = 0; i < numControllers; ++i)
			{
				for (std::vector<Ptr<Socket> >::iterator it = m_vController[i]->m_vSendSocket.begin();
						it != m_vController[i]->m_vSendSocket.end(); ++it)
				{
					if (socket == *it)
					{
						Simulator::Schedule(FlowSetUpDelay(),&Controller::SendFlowMod,m_vController[i],*it,destinationHeader.GetSenderID(),destinationHeader.GetPacketID());
					}
					else if (std::rand()%101/100.0 < (Diameter_controller*Diameter-1.0)/(numVertices*numControllers-1.0) )
					{
						Simulator::Schedule(FlowSetUpDelay(),&Controller::SendFlowMod,m_vController[i],*it,destinationHeader.GetSenderID(),destinationHeader.GetPacketID());
					}
				}
		...
		else if (destinationHeader.GetPacketType() == FlowRemove)
		{
			...
			NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Controller " << m_id << " Receives a flow remove packet!");
			m_num_flow_remove++;
		}
		else if (destinationHeader.GetPacketType() == StatsRes)
		{
			if (destinationHeader.GetSenderID() == m_id && !m_mStatusReq.empty())
			{
		...
		{
			NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Controller " << m_id << " receives a wrong packet");
			//exit (EXIT_FAILURE);
		}
	}
	  ...
	  if (InetSocketAddress::IsMatchingType (from))
	    {
	      ...
	      //m_GlobalSocket[m_id].push_back(s);
	    }
	}
};
...
const char* NumConvertString(std::string type)
{
	...
	std::string file_string = "simulationResult/scalability/p2pGlobal_" + type + "_Diameter=";
	convert << Diameter;
...
std::vector<Controller*> Controller::m_vController;
...
main (int argc, char *argv[])
{
  ...
  std::vector<Controller*> v_ControllerClass;

  for (int i = 0; i < numControllers; ++i)
  {
	  Controller* p1 = new Controller(numVertices, Diameter, Seconds(pullInterval));
	  v_ControllerClass.push_back(p1);
  }

  for (int i = 0; i < numControllers; ++i)
  {
	  Controller::m_vController.push_back(v_ControllerClass[i]);
  }
  // create switches
