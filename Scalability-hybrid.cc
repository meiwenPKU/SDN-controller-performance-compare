 ...
 * hybrid SDN. In hybrid SDN, there are multiple "sub-domain", where each domain is controlled by a hierachical
 * SDN.
 *
...
//--------------define the period for pulling the statistics------------------------
 ...
 int numVertices = 50; // the number of vertices in the network
 ...
 double Diameter_controller = 1.667; // the average number of controllers a flow going through
 int Diameter_domain = 1.2; // the average number of sub-domains a flow going through
 double numSubDomains = 5; // the number of sub-domains
 ...
 double Prob_domain = (Diameter_domain-1.0)/(numSubDomains - 1.0); // the prob of whether a domain (besides the source) is in a path
...
NS_LOG_COMPONENT_DEFINE ("HybridSDN");
...
public:
	...
	uint32_t m_id; // the id of the root controller
...
public:
	...
	m_num_rec_status_req(0),m_num_tr_status_res(0), m_id(0){
	...
	{
	  NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " sends a flow mod packet!");
	  m_num_flow_mod++;
	  MyHeader sourceHeader;
	...
	void SendReq()
	{
				...
				it != m_vSendSocket.end(); ++it)
		{
		    ...
		    p->AddHeader (sourceHeader);
			(*it)->Send(p);
			NS_LOG_INFO( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " sends a status request packet");
			m_num_status_request++;
			m_id_status_req++;
		}
		//m_id_status_req++;
	...
	{
		NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Root controller " << m_id << " Send a status response packet!");
	    MyHeader sourceHeader;
	...
	void HandleRead(Ptr<Socket> socket)
	{
		...
		if (destinationHeader.GetPacketType() == FlowInitial)
		{
			...
			NS_LOG_INFO( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " receives a initial flow setup packet");
			m_num_flow_setup++;
			for (std::vector<Ptr<Socket> >::iterator it = m_vSendSocket.begin();
					it != m_vSendSocket.end(); ++it)
			{
		...
		else if (destinationHeader.GetPacketType() == FlowRemove)
		{
			...
			NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " Receives a flow remove packet!");
			m_num_flow_remove++;
		}
		else if (destinationHeader.GetPacketType() == StatsRes)
		{
			...
			NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " Receives a status report packet!");
			m_num_status_report++;
		}
		else if (destinationHeader.GetPacketType() == StatsReq)
		{
		    //once receive the status request, reply status response packet
		    NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " Received a status request packet!");
		    m_num_rec_status_req++;
		    SendStatsRes(socket,destinationHeader.GetPacketID(),destinationHeader.GetSenderID());
		}
		else
		{
			NS_LOG_INFO ( Simulator::Now().GetSeconds() << " Root Controller " << m_id << " receives a wrong packet");
			//exit (EXIT_FAILURE);
		}
	}
...
public:
	...
	uint32_t m_num_root_rec_status_res; // the number of flow response packet received from the root controller
	uint32_t m_num_root_tr_status_req; // the number of flow request packet sent to the root controller
...
public:
	...
	void SendReq()
	{
		//m_QSwitchStatsReqTime.push(Simulator::Now().GetSeconds());
		//m_QSwitchStatsReqID.push(m_id_switch_status_req);
...
const char* NumConvertString(std::string type)
{
	...
	std::string file_string = "simulationResult/scalability/hybrid_" + type + "_Diameter=";
	convert << Diameter;
	file_string += convert.str() + "_ControllerDiameter=";
	convert.str("");
	convert << Diameter_controller;
	file_string += convert.str() + "_DomainDiameter=";
	convert.str("");
	convert << Diameter_domain;
	file_string += convert.str();

	file_string += "_ControlDataRate=" + LocalControlDataRate;
	file_string += "_numSubDomains=";
	convert.str("");
	convert << numSubDomains;
...
{
	typedef std::vector<NetDeviceContainer> v_NetDeviceContainer;
	typedef std::vector<v_NetDeviceContainer> vv_NetDeviceContainer;
	typedef std::vector<vv_NetDeviceContainer> vvv_NetDeviceContainer;

	typedef std::vector<Ipv4InterfaceContainer> v_Ipv4InterfaceContainer;
	typedef std::vector<v_Ipv4InterfaceContainer> vv_Ipv4InterfaceContainer;
	typedef std::vector<vv_Ipv4InterfaceContainer> vvv_Ipv4InterfaceContainer;

	typedef std::vector<Switch> v_Switch;
	typedef std::vector<v_Switch> vv_Switch;
	typedef std::vector<vv_Switch> vvv_Switch;

	typedef std::vector<LocalController> v_LocalController;
	typedef std::vector<v_LocalController> vv_LocalController;

	typedef std::vector<NodeContainer> v_NodeContainer;
	typedef std::vector<v_NodeContainer> vv_NodeContainer;
  ...
  cmd.AddValue("numSubDomains", "Number of sub-domains in the network", numSubDomains);
  cmd.AddValue("Diameter_domain", "Average number of sub-domains a flow going through", Diameter_domain);
  cmd.AddValue("aver_interval","Average interval between flow arriving", aver_interval);
  cmd.Parse (argc, argv);

  //LogComponentEnable("HybridSDN",LOG_LEVEL_INFO);

  double delayAS = 0.001 * Diameter; // the time for a flow goes through one domain.

  vvv_NetDeviceContainer Switch2ControllerNetDevice(numSubDomains, vv_NetDeviceContainer(numControllers, v_NetDeviceContainer(numVertices)));
  vvv_Ipv4InterfaceContainer Switch2ControllerIpv4Interface(numSubDomains, vv_Ipv4InterfaceContainer(numControllers, v_Ipv4InterfaceContainer(numVertices)));

  vvv_Switch v_SwitchClass(numSubDomains,vv_Switch(numControllers,v_Switch(numVertices)));
  vv_LocalController v_ControllerClass(numSubDomains,v_LocalController(numControllers,LocalController(numVertices, Diameter, Seconds(pullInterval))));

  std::vector<RootController> v_RootControllerClass(numSubDomains,RootController(numControllers, Diameter_controller, Seconds(pullInterval)));
  vv_NetDeviceContainer Local2RootNetDevice(numSubDomains,v_NetDeviceContainer(numControllers));
  vv_Ipv4InterfaceContainer Local2RootIpv4Interface(numSubDomains,v_Ipv4InterfaceContainer(numControllers));

  // create switches
  NS_LOG_INFO (" Create switch nodes.");
  vv_NodeContainer v_switch(numSubDomains,v_NodeContainer(numControllers));
  for (int j = 0; j < numSubDomains; ++j)
  {
	  for (int i = 0; i < numControllers; ++i)
	  {
		  v_switch[j][i].Create(numVertices);
	  }
  }
  for (int k = 0; k < numSubDomains; ++k)
  {
	  v_RootControllerClass[k].m_id = k;
	  for (int i = 0; i < numControllers; ++i)
	  {
		  for (int j = 0; j < numVertices; ++j)
		  {
			  v_SwitchClass[k][i][j].m_id = k * numControllers * numVertices + i * numVertices + j;
		  }
	  }
  }
  ...
  v_NodeContainer v_Controller(numSubDomains);
  for (int i = 0; i < numSubDomains; ++i)
  {
	  v_Controller[i].Create(numControllers);
  }
  ...
  for (int j = 0; j < numSubDomains; ++j)
  {
	  for (int i = 0; i < numControllers; i++)
	  {
		  v_ControllerClass[j][i].m_id = j*numControllers + i;
	  }
  }

  NodeContainer v_RootController;
  v_RootController.Create(numSubDomains);
  ...
  for (int i = 0; i < numSubDomains; ++i)
  {
	  ...
	  {
		  internet.Install(v_switch[i][j]);
	  }
  }
  for (v_NodeContainer::iterator it = v_Controller.begin(); it != v_Controller.end(); ++it)
  {
	  internet.Install(*it);
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  ...
		  {
			  Switch2ControllerNetDevice[k][j][i] = p2p.Install(v_switch[k][j].Get(i),v_Controller[k].Get(j));
		  }
	  }
  }
  // connect the local controller to the root controller
  for (int j = 0; j < numSubDomains; ++j)
  {
	  for (int i = 0; i < numControllers; ++i)
	  {
		  Local2RootNetDevice[j][i] = p2p_root.Install(v_Controller[j].Get(i),v_RootController.Get(j));
	  }
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  for (int i = 0; i < numVertices; ++i)
		  {
			  ...
			  Switch2ControllerIpv4Interface[k][j][i] = ipv4.Assign(Switch2ControllerNetDevice[k][j][i]);
		  }
	  }
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  ...
	  {
		  oss.str ("");
		  oss << numSubDomains + k + 1 << ".0." << i << ".0";
		  ipv4.SetBase (oss.str ().c_str (), "255.255.255.0");
		  Local2RootIpv4Interface[k][i] = ipv4.Assign(Local2RootNetDevice[k][i]);
	  }
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int i = 0; i < numControllers; ++i)
	  {
		  Ptr<Socket> socket = Socket::CreateSocket (v_Controller[k].Get(i), tid);
		  v_ControllerClass[k][i].StartController(socket);
	  }
  }
  for (int i = 0; i < numSubDomains; ++i)
  {
	  Ptr<Socket> socket = Socket::CreateSocket (v_RootController.Get(i), tid);
	  v_RootControllerClass[i].StartRootController(socket);
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  for (int i = 0; i < numVertices; ++i)
		  {
			  Ptr<Socket> socket = Socket::CreateSocket (v_switch[k][j].Get(i), tid);
			  Ipv4Address t_local =  Switch2ControllerIpv4Interface[k][j][i].GetAddress(0);
			  Ipv4Address t_remote = Switch2ControllerIpv4Interface[k][j][i].GetAddress(1);
			  v_SwitchClass[k][j][i].EstablishControllerConnection(socket, t_local, t_remote);
		  }
	  }
  }
  ...
  for (int j = 0; j < numSubDomains; ++j)
  {
	  for (int i = 0; i < numControllers; ++i)
	  {
		  Ptr<Socket> socket = Socket::CreateSocket (v_Controller[j].Get(i), tid);
		  Ipv4Address t_local =  Local2RootIpv4Interface[j][i].GetAddress(0);
		  Ipv4Address t_remote = Local2RootIpv4Interface[j][i].GetAddress(1);
		  v_ControllerClass[j][i].EstablishRootControllerConnection(socket, t_local, t_remote);
	  }
  }
  ...
  for (int m = 0; m < numSubDomains; ++m)
  {
	  for (int k = 0; k < numControllers; ++k)
	  {
		  for (int i = 0; i < numVertices; ++i)
		  {
			  double scheduletime = 500 + i;
			  for (uint32_t j = 0; j < N_flow; ++j)
			  {
				  Simulator::ScheduleWithContext(v_SwitchClass[m][k][i].m_SendSocket->GetNode()->GetId(),
						  MilliSeconds(scheduletime),&Switch::SendPacket, &v_SwitchClass[m][k][i],FlowInitial);
				  double delay = delayAS;
				  for (int l = 0; l < numSubDomains; ++l)
				  {
					  if (l != k && std::rand()%101/100.0 < Prob_domain)
					  {
						  // the flow will go through the domain
						  int source_switch = std::rand()%numVertices;
						  int source_local = std::rand()%numControllers;
						  Simulator::ScheduleWithContext(v_SwitchClass[l][source_local][source_switch].m_SendSocket->GetNode()->GetId(),
								  MilliSeconds(scheduletime + delay),&Switch::SendPacket, &v_SwitchClass[l][source_local][source_switch],FlowInitial);
						  delay += delayAS;
					  }
				  }
  ...
  for (int j = 0; j < numSubDomains; ++j)
  {
	  for (int i = 0; i < numControllers; ++i)
	  {
		  Simulator::ScheduleWithContext(v_Controller[j].Get(i)->GetId(),Seconds(1.0), &LocalController::SendReq, &v_ControllerClass[j][i]);
		  Simulator::ScheduleWithContext(v_Controller[j].Get(i)->GetId(),Seconds(1.1), &LocalController::SendRootReq, &v_ControllerClass[j][i]);
	  }
  }
  ...
  for (int i = 0; i < numSubDomains; ++i)
  {
	  Simulator::ScheduleWithContext(v_RootController.Get(i)->GetId(), Seconds(1.2), &RootController::SendReq, &v_RootControllerClass[i]);
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  for (int i = 0; i < numVertices; ++i)
		  {
			  Simulator::ScheduleWithContext(v_SwitchClass[k][j][i].m_SendSocket->GetNode()->GetId(),
					  Seconds(timeout-1),&Switch::CheckFlow, &v_SwitchClass[k][j][i]);
		  }
	  }
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  ...
	  for (int j = 0; j < numControllers; ++j)
	  {
		  ...
		  local_controller_sum_packets[0] += v_ControllerClass[k][j].m_num_root_flow_setup;
		  local_controller_sum_packets[1] += v_ControllerClass[k][j].m_num_root_flow_mod;
		  local_controller_sum_packets[2] += v_ControllerClass[k][j].m_num_root_rec_status_req;
		  local_controller_sum_packets[3] += v_ControllerClass[k][j].m_num_root_rec_status_res;
		  local_controller_sum_packets[4] += v_ControllerClass[k][j].m_num_root_tr_status_req;
		  local_controller_sum_packets[5] += v_ControllerClass[k][j].m_num_root_tr_status_res;

		  local2root_delay_file << "Local Controller " << k*numControllers+j << "\n";
		  for (std::vector<double>::iterator it = v_ControllerClass[k][j].m_vRootStatsReqDelay.begin();
				  it != v_ControllerClass[k][j].m_vRootStatsReqDelay.end(); ++it)
		  {
			  local2root_delay_file << *it << " ";
		  }
		  local2root_delay_file << "\n";

		  local2switch_delay_file << "Local Controller " << k*numControllers+j << "\n";
		  for (std::vector<double>::iterator it = v_ControllerClass[k][j].m_vSwitchStatsReqDelay.begin();
				  it != v_ControllerClass[k][j].m_vSwitchStatsReqDelay.end(); ++it)
		  {
			  local2switch_delay_file << *it << " ";
		  }
		  local2switch_delay_file << "\n";

		  std::cout << "Local Controller " << k*numControllers+j << std::endl;
		  std::cout << v_ControllerClass[k][j].m_num_flow_mod * SIZE_PKT_FLOW_MOD << " "
				  << v_ControllerClass[k][j].m_num_status_request * SIZE_PKT_STATS_REQ << " "
				  << v_ControllerClass[k][j].m_num_flow_remove * SIZE_PKT_FLOW_REMOVE << " "
				  << v_ControllerClass[k][j].m_num_flow_setup * SIZE_PKT_INITIAL_FLOW << " "
				  << v_ControllerClass[k][j].m_num_status_report * SIZE_PKT_STATS_RES << " "
				  << v_ControllerClass[k][j].m_num_root_flow_setup * SIZE_PKT_INITIAL_FLOW << " "
				  << v_ControllerClass[k][j].m_num_root_flow_mod * SIZE_PKT_FLOW_MOD << " "
				  << v_ControllerClass[k][j].m_num_root_rec_status_req * SIZE_PKT_STATS_REQ << " "
				  << v_ControllerClass[k][j].m_num_root_rec_status_res * SIZE_PKT_STATS_RES << " "
				  << v_ControllerClass[k][j].m_num_root_tr_status_req * SIZE_PKT_STATS_REQ << " "
				  << v_ControllerClass[k][j].m_num_root_tr_status_res * SIZE_PKT_STATS_RES << " ";
		  Statistics(v_ControllerClass[k][j].m_vFlowSetupDelay,average, std);
		  std::cout << average << " " << std << " ";
		  Statistics(v_ControllerClass[k][j].m_vRootStatsReqDelay,average, std);
		  std::cout << average << " " << std << " ";
		  Statistics(v_ControllerClass[k][j].m_vSwitchStatsReqDelay,average, std);
		  std::cout << average << " " << std << std::endl;

		  flow_file << "Local Controller " << k*numControllers+j << "\n";

		  int switch_sum_packet[5] = {};
		  for (int i = 0; i < numVertices; ++i)
		  {
			  switch_sum_packet[0] += v_SwitchClass[k][j][i].m_num_flow_mod;
			  switch_sum_packet[1] += v_SwitchClass[k][j][i].m_num_status_request;
			  switch_sum_packet[2] += v_SwitchClass[k][j][i].m_num_flow_remove;
			  switch_sum_packet[3] += v_SwitchClass[k][j][i].m_num_flow_setup;
			  switch_sum_packet[4] += v_SwitchClass[k][j][i].m_num_status_report;

			 // std::cout << "Switch " << i << std::endl;
			  std::cout << v_SwitchClass[k][j][i].m_num_flow_mod * SIZE_PKT_FLOW_MOD << " "
					  << v_SwitchClass[k][j][i].m_num_status_request * SIZE_PKT_STATS_REQ << " "
					  << v_SwitchClass[k][j][i].m_num_flow_remove * SIZE_PKT_FLOW_REMOVE << " "
					  << v_SwitchClass[k][j][i].m_num_flow_setup * SIZE_PKT_INITIAL_FLOW << " "
					  << v_SwitchClass[k][j][i].m_num_status_report * SIZE_PKT_STATS_RES << " "
					  << v_SwitchClass[k][j][i].m_numFlow << " ";
			  Statistics(v_SwitchClass[k][j][i].m_vDelay,average, std);
			  std::cout << average << " " << std << std::endl;

			  for (std::vector<uint32_t>::iterator it = v_SwitchClass[k][j][i].m_vNumFlow.begin();
					  it != v_SwitchClass[k][j][i].m_vNumFlow.end(); ++it)
			  {
				  flow_file << *it << " ";
			  }
			  flow_file << "\n";

			  FlowSetup_file << "Local Controller " << k*numControllers+j << " Switch " << i << "\n";
			  for (std::vector<double>::iterator it = v_SwitchClass[k][j][i].m_vDelay.begin();
					  it != v_SwitchClass[k][j][i].m_vDelay.end();++it)
			  {
				  FlowSetup_file << *it << " ";
			  }
			  FlowSetup_file << "\n";
		  ...
		  std::cout << switch_sum_packet[0]/(v_ControllerClass[k][j].m_num_flow_mod + 0.0) << " "
				  << switch_sum_packet[1]/(v_ControllerClass[k][j].m_num_status_request + 0.0) << " "
				  << switch_sum_packet[2]/(v_ControllerClass[k][j].m_num_flow_remove + 0.0)<< " "
				  << switch_sum_packet[3]/(v_ControllerClass[k][j].m_num_flow_setup + 0.0) << " "
				  << switch_sum_packet[4]/(v_ControllerClass[k][j].m_num_status_report + 0.0)<< std::endl;
		  std::cout << "*********************************************" << std::endl;
	  }

	  root2local_delay_file << "Root controller " << k << "\n";
	  for (std::vector<double>::iterator it = v_RootControllerClass[k].m_vDelay.begin();
			  it != v_RootControllerClass[k].m_vDelay.end(); ++it)
	  {
		  root2local_delay_file << *it << " ";
	  }
	  root2local_delay_file << "\n";

	  std::cout << "Root controller " << k << std::endl;
	  std::cout << v_RootControllerClass[k].m_num_flow_mod * SIZE_PKT_FLOW_MOD << " "
			  << v_RootControllerClass[k].m_num_status_request * SIZE_PKT_STATS_REQ << " "
			  << v_RootControllerClass[k].m_num_flow_remove * SIZE_PKT_FLOW_REMOVE << " "
			  << v_RootControllerClass[k].m_num_flow_setup * SIZE_PKT_INITIAL_FLOW << " "
			  << v_RootControllerClass[k].m_num_status_report * SIZE_PKT_STATS_RES << " "
			  << v_RootControllerClass[k].m_num_rec_status_req * SIZE_PKT_STATS_REQ << " "
			  << v_RootControllerClass[k].m_num_tr_status_res * SIZE_PKT_STATS_RES << " ";
	  Statistics(v_RootControllerClass[k].m_vDelay,average, std);
	  std::cout << average << " " << std << std::endl;

	  std::cout << "packet loss rate" << std::endl;
		  std::cout << local_controller_sum_packets[0]/(v_RootControllerClass[k].m_num_flow_setup + 0.0) << " "
				  << local_controller_sum_packets[1]/(v_RootControllerClass[k].m_num_flow_mod + 0.0) << " "
				  << local_controller_sum_packets[2]/(v_RootControllerClass[k].m_num_status_request + 0.0)<< " "
				  << local_controller_sum_packets[3]/(v_RootControllerClass[k].m_num_tr_status_res + 0.0) << " "
				  << local_controller_sum_packets[4]/(v_RootControllerClass[k].m_num_rec_status_req + 0.0)<< " "
				  << local_controller_sum_packets[5]/(v_RootControllerClass[k].m_num_status_report + 0.0)<< std::endl;
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  for (int i = 0; i < numVertices; ++i)
		  {
			  v_setup_delay.insert(v_setup_delay.end(),v_SwitchClass[k][j][i].m_vDelay.begin(), v_SwitchClass[k][j][i].m_vDelay.end());
		  }
	  }
  }
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  v_setup_delay.insert(v_setup_delay.end(),v_RootControllerClass[k].m_vDelay.begin(), v_RootControllerClass[k].m_vDelay.end());
  }
  Statistics(v_setup_delay,average, std);
  std::cout << average << " " << std << " ";
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  v_setup_delay.insert(v_setup_delay.end(),v_ControllerClass[k][j].m_vFlowSetupDelay.begin(), v_ControllerClass[k][j].m_vFlowSetupDelay.end());
	  }
  }
  Statistics(v_setup_delay,average, std);
  std::cout << average << " " << std << " ";
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  v_setup_delay.insert(v_setup_delay.end(),v_ControllerClass[k][j].m_vRootStatsReqDelay.begin(), v_ControllerClass[k][j].m_vRootStatsReqDelay.end());
	  }
  }
  Statistics(v_setup_delay,average, std);
  std::cout << average << " " << std << " ";
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  v_setup_delay.insert(v_setup_delay.end(),v_ControllerClass[k][j].m_vSwitchStatsReqDelay.begin(), v_ControllerClass[k][j].m_vSwitchStatsReqDelay.end());
	  }
  }
  Statistics(v_setup_delay,average, std);
  std::cout << average << " " << std << " ";
  ...
  for (int k = 0; k < numSubDomains; ++k)
  {
	  for (int j = 0; j < numControllers; ++j)
	  {
		  for (int i = 0; i < numVertices; ++i)
		  {
			  v_flow_num.push_back(double(v_SwitchClass[k][j][i].m_numFlow));
		  }
	  }
  }
